...
function parseVaultState(data: Buffer, address: string): KaminoVaultOnChain | null {
  try {
    ...
    // If still no name, generate from curator + token mint
    if (!name || name.length < 3) {
      const symbol = getTokenSymbol(tokenMint);
      const curatorName = KNOWN_ADMIN_TO_CURATOR[admin];
      name = curatorName ? `${curatorName} ${symbol}` : `Kamino ${symbol} Vault`;
    }
